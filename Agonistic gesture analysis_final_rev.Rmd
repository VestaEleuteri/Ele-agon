...
table_mod_rcp_va2 %>%
  ...
  kable(
    format = "html",
    caption = "Frequency of gesture modality by recipient visual attention"
  ) %>%
  kable_styling("striped", full_width = FALSE) %>%
  column_spec(1, bold = TRUE) %>%
  column_spec(2:ncol(table_mod_rcp_va2) + 1, width = "100px")
...
bout_data_resp <- bout_data %>%
  filter(!Response_waiting %in% c("Unk", "NV")) %>% #Remove Unk or NV
  droplevels()
```
...
We checked which gesture types used during agonistic communications met the intentionality criteria by excluding those that never showed goal-directedness: never used during bout parts of communications where the goal was met. 
...
dropped_levels_gu
```
**Seven gesture types were not associated with goal-directedness so they were not considered intentionally produced**
...
xdata_int <- xdata %>%
  filter(
    !(Gesture_type %in% c("Body Lean backward", "Front limb Swing", "Head Lower", "Tail Raise", "Ear Hit object", "Tusk Touch recipient", "Front limb Stand-On"))
  )  %>% 
  droplevels()

# Remove subset of gesture types not used at least 3 times
xdata_int_min3_gtype <- xdata_int %>%
  group_by(Gesture_type) %>%
  filter(n() >= 3) %>%
  ungroup() %>%
  droplevels()

# Number of gesture tokens for dataset of types used at least 3 times
data.frame(`Number of tokens` = nrow(xdata_int_min3_gtype)) %>%
  setNames("Number of gesture tokens") %>%  
  kable(format = "html", caption = "") %>%
  kable_styling("striped", full_width = FALSE)

# Number of gesture tokens per population
xdata_int_min3_gtype %>%
  count(Population, name = "Number of gesture tokens") %>%
  kable(format = "html", caption = "") %>%
  kable_styling("striped", full_width = FALSE)
...
data.frame(`Number of gesture types used at least 3 times` = 
  n_distinct(xdata_int_min3_gtype$Gesture_type))  %>% 
...
data.frame(`Number of gesture actions used at least 3 times` = 
  n_distinct(xdata_int_min3_gtype$Gesture_action))  %>% 
...
remaining_levels <- levels(xdata_int_min3_gtype$Gesture_type)
...
bp_gesture <-xdata_int_min3_gtype %>%
 count(Body_part_2, name = "Frequency")  %>%
  mutate(Percentage = round((Frequency / sum(Frequency)) * 100, 2)) %>%
  rename(`Body part` = Body_part_2)
...
# Check number of gesture actions by body part 
xdata_int_min3_gtype %>%
  group_by(Body_part_2) %>%
  summarise(`Number of gesture types` = n_distinct(Gesture_action)) %>%
  rename(`Body part` = Body_part_2)  %>%
  kable(format = "html", 
        caption = "Number of intentional gesture actions per body part") %>%
  kable_styling("striped", full_width = FALSE)
...
gesture_act_type_pop <- xdata_int_min3_gtype %>%
 count(Gesture_action, Gesture_type, name = "Frequency") %>%
  arrange(Gesture_action, Gesture_type) %>%
  group_by(Gesture_action) %>%
  mutate(`Gesture action frequency` = sum(Frequency)) %>%
...
## Make plot actions
gact_min3 <- ggplot(xdata_int_min3_gtype, aes(x = as.character(Gesture_action), fill = Population)) +
  geom_bar() + 
  ...
  scale_fill_scico_d(
   palette = "navia",
    begin = 0.2,
    end = 0.7) + theme(axis.text.x=element_text(angle=90, hjust=1, vjust=.5)) + scale_y_continuous(breaks = seq(0, max(table(xdata_int_min3_gtype$Gesture_action)), 25)) 
...
gtype_min3 <- ggplot(
  ...
  scale_y_continuous(breaks = seq(0, max(table(xdata_int_min3_gtype$Gesture_type)), 25)) +
...
cum_plot_data_2 <- xdata_int_min3_gtype %>%
  group_by(Population) %>%
  arrange(Com_number, .by_group = TRUE) %>%
  droplevels() %>%
...
```
**The repertoire reaches asymptote in Addo but only approaches in Marakele**
...
xdata_int_addo_gtype <- subset(xdata_int, Population %in% "Addo") %>% 
  droplevels() 

# Repertoire of Addo gesture actions
gtype_counts_addo <- xdata_int_addo_gtype %>%
...
gtype_counts_addo <- gtype_counts_addo %>%
  mutate(
    Total = sum(Frequency),
...
types_70_addo <- gtype_counts_addo %>%
...
xdata_int_mar_gtype <- subset(xdata_int, Population %in% "Marakele") %>% 
  droplevels() 

# Repertoire of Marakele gesture actions
gtype_counts_mar <- xdata_int_mar_gtype %>%
  mutate(Gesture_type = droplevels(Gesture_type)) %>%
  count(Gesture_type, name = "Frequency") %>%
  arrange(desc(Frequency)) %>%
  rename(`Gesture type` = Gesture_type)
...
## Calculate percentages and cumulative percentages of actions
gtype_counts_mar <- gtype_counts_mar %>%
  mutate(
   Total = sum(Frequency),
    Percentage = round(100 * Frequency / sum(Frequency), 2), 
    Cumulative_percentage = round(cumsum(Percentage), 2)
...
**The populations shared three out of of their 9 frequently used gesture types.**

# Goals of elephant agonistic gestural communications

We then checked the goals (i.e., meanings) of elephant gestural communications during agonistic interactions across populations.

To do so, we only included successful gestural communication events where the goal was met. 
...
xdata_goals <- xdata %>%
  filter(!Outcome %in% c("Exclude", "Unclear_exc", "Other_rcp", "Unable", "Unk"))%>%
  droplevels() %>%
    filter(!Goal %in% c("Unk")) 

# Number of communication events
data.frame(`Number of communications` = n_distinct(xdata_goals$Com_number)) %>%
  setNames("Number of gestural communication events") %>%  
  kable(format = "html", caption = "") %>%
  kable_styling("striped", full_width = FALSE)

# Relevel goals for tables
xdata_goals <- xdata_goals %>%
  mutate(
    Goal = case_when(
    Goal %in% c("Move_away_water") ~ "Move away from water",
    Goal %in% c("Move_away_reeds") ~ "Move away from reeds",
    Goal %in% c("Move_away") ~ "Move away",
    Goal %in% c("Stop_behaviour") ~ "Stop behaviour",
    Goal %in% c("Stop_behaviour_water") ~ "Stop behaviour at water",
    Goal %in% c("Stop_behaviour_reeds") ~ "Stop behaviour at reeds",
    TRUE ~ Goal
    )
  )
...
xdata_goals <- xdata_goals %>%
  mutate(
    Goal_2 = case_when(
    Goal %in% c("Move away from water") ~ "Move away",
    Goal %in% c("Move away from reeds") ~ "Move away",
    Goal %in% c("Move away") ~ "Move away",
    TRUE ~ "Stop behaviour"
    )
  )
...
xdata_goals %>%
  ...
  count(Goal_2, name = "Number of gestural communication events") %>%
  mutate(Percentage = round((`Number of gestural communication events`/ sum(`Number of gestural communication events`)) * 100, 1)) %>% 
  rename(`Goal` = Goal_2) %>%
  kable(format = "html", caption = "") %>%
  kable_styling("striped", full_width = FALSE) %>%
  column_spec(1, bold = TRUE) 
...
# Goals and recipient context before 

Then to understand the specific function of the goals we checked the recipient's context before each goal type

```{r goals-rcp-context}
# Rename contexts and remove contexts with less than three communication events
valid_ctx_goal <- xdata_goals %>%
  mutate(
    Rcp_prior_context = case_when(
      Rcp_prior_context == "Agonistic_con" ~ "Agonistic",
      Rcp_prior_context == "Agonistic_con_water" ~ "Agonistic",
      Rcp_prior_context == "Feeding_mpol" ~ "Feeding (monopolisable food)",
      Rcp_prior_context == "Mud_bath" ~ "Mud bathing",
      Rcp_prior_context == "Social_context_3p" ~ "Social context with third party",
      Rcp_prior_context == "Travel_move" ~ "Travel (move)",
      TRUE ~ Rcp_prior_context
    )
  ) %>%
  filter(!Rcp_prior_context %in% c("Unk", "Unclear")) %>%
  distinct(Com_number, Rcp_prior_context, Goal_2) %>%
  count(Rcp_prior_context, Goal_2, name = "n_comms") %>%
  filter(n_comms >= 3) %>%
  select(Rcp_prior_context, Goal_2)

xdata_goals_ctx <- xdata_goals %>%
  mutate(
    Rcp_prior_context = case_when(
      Rcp_prior_context == "Agonistic_con" ~ "Agonistic",
      Rcp_prior_context == "Agonistic_con_water" ~ "Agonistic",
  ...
  filter(!Rcp_prior_context %in% c("Unk", "Unclear")) %>%
  semi_join(valid_ctx_goal, by = c("Rcp_prior_context", "Goal_2")) %>%
  droplevels()

# Number of communication events per context
data.frame(`Number of communications` = n_distinct(xdata_goals_ctx$Com_number)) %>%
  setNames("Number of gestural communication events") %>%  
  kable(format = "html", caption = "") %>%
  kable_styling("striped", full_width = FALSE)

# Goal context 
goals_rcp_ctx_table <- xdata_goals_ctx %>%
  distinct(Com_number, Rcp_prior_context, Goal_2) %>%
  count(Rcp_prior_context, Goal_2, name = "Number of gestural communication events") %>%
  group_by(Goal_2) %>%
  mutate(
    Percentage = round(
      (`Number of gestural communication events` / sum(`Number of gestural communication events`)) * 100, 1
    )
  ) %>%
  rename(`Recipient prior context` = Rcp_prior_context, `Goal` = Goal_2)

goals_rcp_ctx_table %>%
  kable(
    format = "html",
    caption = "Number of gestural communication events per general goal and recipient prior context"
  ) %>%
  kable_styling("striped", full_width = FALSE) %>%
  column_spec(1, bold = TRUE) %>%
  column_spec(2:4, width = "120px")

write.csv(goals_rcp_ctx_table, "goals_rcp_ctx.csv", row.names = FALSE) # save for table for paper
```

# Goals of elephant gesture types

We then explored the goals of each gesture type observed at least three times during successful agonistic gestural communication events 

```{r goals-gest}
# All goals per all gesture types min 3 times
xdata_goals_gtype_min3 <- xdata_goals %>%
  group_by(Gesture_type) %>%
  filter(n() >= 3) %>%
  ungroup() %>%
  droplevels()

goals_gtype_table <- xdata_goals_gtype_min3 %>%
...
write.csv(goals_gtype_table, "goals_gtypes_all.csv", row.names = FALSE) # save for table for paper

ggplot(goals_gtype_table, aes(x = as.character(`Gesture type`), y = Percentage, fill = Goal)) +
  ...
  scale_y_continuous(breaks = seq(0, max(table(xdata_goals_gtype_min3$Gesture_type)), 25)) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = .5))
...
# Goals of elephant gesture types and recipient prior context

```{r goals-gest-ctx}
# Remove gesture types with less than 3 tokens in context communications above
xdata_goals_ctx_gtype_min3 <- xdata_goals_ctx %>%
  group_by(Gesture_type) %>%
  filter(n() >= 3) %>%
  ungroup() %>%
  droplevels()

# Number of communication events included
data.frame(`Number of communications` = n_distinct(xdata_goals_ctx_gtype_min3$Com_number)) %>%
  setNames("Number of communications") %>%  
  kable(format = "html", caption = "") %>%
  kable_styling("striped", full_width = FALSE)

# All goals per all gesture types (context)
goals_ctx_gtype_min3_table <- xdata_goals_ctx_gtype_min3 %>%
  count(Rcp_prior_context, Gesture_type, Goal, name= "Frequency") %>%
  group_by(Gesture_type) %>%
  mutate(Percentage = round(100 * Frequency / sum(Frequency), 2)) %>%
  ungroup() %>%
  rename(
    `Gesture type` = Gesture_type,
    `Recipient prior context` = Rcp_prior_context,
    `Goal (specific)` = Goal) %>%
  select(`Gesture type`, everything())   %>%
  arrange(`Gesture type`)

goals_gtype_table_ctx <- goals_ctx_gtype_min3_table %>%
  kable(format = "html", caption = "Number of goals per gesture type and context") %>%
  kable_styling("striped", full_width = FALSE) %>%
  column_spec(1, bold = TRUE) %>%
  column_spec(2:3, width = "120px")
goals_gtype_table_ctx

write.csv(goals_ctx_gtype_min3_table, "goals_gtypes_rcpctx.csv", row.names = FALSE) # save for table for paper
```

# Population differences in goals

```{r goals-pop}
# Number of communications with goal
data.frame(`Number of communications` = n_distinct(xdata_goals$Com_number)) %>%
  setNames("Number of gestural communication events") %>%  
  kable(format = "html", caption = "") %>%
  kable_styling("striped", full_width = FALSE)

# Number of communications with goal per population
xdata_goals %>%
  distinct(Com_number, Population) %>%
  count(Population, name = "Number of gestural communication events") %>%
  kable(format = "html", caption = "") %>%
  kable_styling("striped", full_width = FALSE)

# Goal types per population
goals_table_pop <- xdata_goals %>%
  distinct(Population, Com_number, Goal) %>%
  count(Population, Goal, name = "Number of gestural communication events") %>%
  group_by(Population) %>%
  mutate(
    Percentage = round(
      (`Number of gestural communication events` / sum(`Number of gestural communication events`)) * 100, 1
...
goals_table_pop %>%
  kable(format = "html", caption = "Number of gestural communication events per goal and population") %>%
  kable_styling("striped", full_width = FALSE) %>%
  column_spec(1, bold = TRUE) %>%
  column_spec(2:3, width = "120px")
...
# Goal types per population and context (REMOVE?)
goals_rcp_ctx_table_pop <- xdata_goals_ctx %>%
  distinct(Com_number, Population, Rcp_prior_context, Goal) %>%
  count(Goal, Rcp_prior_context, Population, name = "Number of gestural communication events") %>%
    filter(!Goal %in% c("Unk")) %>%
  rename(
    `Recipient prior context` = Rcp_prior_context,
    `Goal (specific)` = Goal)

goals_rcp_ctx_table_pop %>%
  kable(format = "html", caption = "Number of gestural communication events per recipient prior context in each population") %>%
  kable_styling("striped", full_width = FALSE) %>%
  column_spec(1, bold = TRUE) %>%
  column_spec(2:3, width = "120px")

write.csv(goals_rcp_ctx_table_pop, "goals_pop_rcp_ctx.csv", row.names = FALSE) # save for table for paper
```  
...
GLMM_outcome_pop=glmer(Outcome_dc ~ Population + (1|Pop_Signaller),
            data=bout_data_outcome_id, family=binomial, nAGQ = 0,
